...
> - **EDAS Link**: [Paper #1571105900](https://www.edas.info/showPaper.php?m=1571105900)
...
- **성능 최적화**:
    - 컴포넌트 언마운트(Unmount) 시 `EventSource` 리소스를 명확히 해제하여 메모리 누수를 방지했습니다.
    - `Recoil` 상태 업데이트를 최소화하여 불필요한 리렌더링을 방지하고 성능을 최적화했습니다.
- **데이터 시각화**: (Recharts 활용)
...
- **UX (사용자 경험) 개선**:
    ...
    - `useRef`를 활용하여 키보드가 활성화될 때 **입력 필드가 가려지지 않도록 화면을 자동으로 스크롤**하는 기능을 구현, 사용자 입력 편의성을 극대화했습니다.
